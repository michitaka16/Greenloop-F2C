...
# ---------------------------------------------------------------------------
from adoptakale.dashboard.components.task_list import render_today_actions  # noqa: E402
from adoptakale.dashboard.design_decisions import render_design_decisions_panel  # noqa: E402
from adoptakale.data.loader import (  # noqa: E402
    load_crops,
    load_shipments,
    load_staff,
)
from adoptakale.data.ema import load_live_electricity  # noqa: E402
...
# The `adoptakale` CLI handles the libomp dylib lookup before this point.
from adoptakale.layer1.features import build_features  # noqa: E402
from adoptakale.layer1.model import load_models, train_models  # noqa: E402
from adoptakale.layer1.predict import predict_demand  # noqa: E402
from adoptakale.layer2.exceptions import InfeasibleError  # noqa: E402
from adoptakale.layer2.feasibility_check import validate_constraints  # noqa: E402
from adoptakale.layer2.objective import MODE_LABELS, ObjectiveWeights  # noqa: E402
from adoptakale.layer2.optimizer import build_and_solve  # noqa: E402
from adoptakale.layer2.forecast_band import compute_profit_band  # noqa: E402
from adoptakale.layer2.scenarios import (  # noqa: E402
    TyphoonScenarioInput,
    apply_typhoon,
    compare_plans,
)
from adoptakale.layer2.sustainability import (  # noqa: E402
    compute_sustainability_kpis,
    compute_weekly_sustainability,
)
from adoptakale.layer3.environment import HydroFarmEnv  # noqa: E402
from adoptakale.layer3.autonomy_gate import AutonomyGate, AutonomyMode  # noqa: E402
from adoptakale.layer1b.simulation import (  # noqa: E402
    GROWTH_BADGES,
    NUTRITION_BADGES,
...
from adoptakale.llm import LLMUnavailable, explain_plan, llm_is_configured  # noqa: E402
from adoptakale.rag.agent import RAGAgent  # noqa: E402

# Governance — Deployment Gate
from adoptakale.governance.deployment_gate import (  # noqa: E402
    DeploymentDecision,
    GateResult,
    GateStatus,
    evaluate_all_gates,
)
from adoptakale.governance.implications_audit import (  # noqa: E402
    ImpactSeverity,
    ImplicationCategory,
...
from adoptakale.monitoring.drift_detector import (  # noqa: E402
    DriftSeverity,
    DriftType,
    get_all_drift_checks,
    run_all_checks,
)
from adoptakale.rag.hitl import (  # noqa: E402
    Tone,
    TONE_LABELS,
    FeedbackRecord,
    QueryRecord,
...
def _get_live_electricity_df():
    ...
    if "live_electricity_df" not in st.session_state:
        try:
            from adoptakale.data.ema import load_live_electricity
...
def _get_today_shopee_orders() -> tuple[float, str]:
    ...
    if "shopee_orders_cache" not in st.session_state:
        try:
            from adoptakale.data.loader import load_orders
            df = load_orders()
...
def _get_today_staff_summary() -> tuple[int, int, str]:
    ...
    try:
        from adoptakale.data.loader import load_staff
        staff_df = load_staff()
        total = len(staff_df)
...
try:
    from adoptakale.layer3.agent import HydroFarmAgent  # noqa: E402
...
def get_weekly_sustainability(crops, electricity_df, staff):
    ...
    """
    from adoptakale.layer1.features import build_features as build_feats
    from adoptakale.layer1.model import load_models, train_models
    from adoptakale.layer1.predict import predict_demand as _predict
    from adoptakale.layer1b.simulation import diagnose_all_racks_simulated
...
def main():
    ...
    if plan:
        from adoptakale.data.shared_data import save_farm_output
...
# Media AI Chatbot
# ---------------------------------------------------------------------------
_ADMIN_PASSWORD = "adoptakale-admin"
