"""Dashboard-facing helpers around `greenloop.utils.llm.chat`.

The low-level client (provider switching, SDK selection, logging) lives
in `greenloop.utils.llm`. This module only exposes what the Streamlit
code needs:
...
def llm_is_configured() -> bool:
    ...
    Delegates to `greenloop.utils.llm._provider_config` so there's one
    source of truth for what "configured" means.
    """
    from dotenv import load_dotenv

    from greenloop.utils.llm import _provider_config
